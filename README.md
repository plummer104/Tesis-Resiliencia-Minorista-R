...
2. [Estructura lógica del entorno de replicación](#2-estructura-lógica-del-entorno-de-replicación)
...
## 2. Estructura lógica del entorno de replicación

**Nota importante.** El repositorio versionado en GitHub contiene únicamente las carpetas de scripts (`Chapter1/`, `Chapter2/`, `Chapter3/`), además de `.gitignore` y `README.md`. Las carpetas `1_Data/` y `3_Outputs/` **no forman parte del contenido versionado del repositorio**, sino que deben crearse localmente en el entorno del replicador para alojar insumos y resultados de ejecución.

Esta convención coincide con las rutas codificadas en los scripts mediante `here::here()` y no debe modificarse salvo que también se actualicen las rutas dentro del código.

### 2.1 Estructura del repositorio en GitHub
...
### 2.2 Estructura local esperada para ejecutar la replicación
...
│   └── 03_create_figures.R
│
├── 1_Data/                      # Directorio local de insumos (creado por el replicador)
│   ├── ipums/
│   ├── excel/
...
│
├── 3_Outputs/                   # Directorio local de salidas (generado al ejecutar)
│   ├── 1_Data_Processed/
│   ├── 2_Models/
│   ├── 3_Figures/
│   └── 4_Tables/
...
Para ejecutar correctamente los scripts, el replicador debe crear localmente las carpetas `1_Data/` y `3_Outputs/` con la estructura indicada arriba. Esas carpetas no necesitan formar parte del repositorio público y no deben confundirse con el contenido efectivamente versionado en GitHub.
...
- **No versionar `1_Data/`** en el repositorio público; ese directorio debe crearse únicamente en el entorno local de ejecución.
- **No subir** extractos de IPUMS USA ni derivados directos de esos microdatos.
- **No versionar `3_Outputs/`** en el repositorio público; las salidas deben generarse y conservarse localmente, y solo las tablas o figuras finales destinadas a difusión deben exportarse de forma selectiva fuera de ese directorio si se desea compartirlas.
